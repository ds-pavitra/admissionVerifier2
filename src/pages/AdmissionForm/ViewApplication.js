...
const ViewApplication = ({ application, fetchlist, toggle, verificationStatus, verificationRemark, verifyType }) => {
    const [profile, setProfile] = useState(null);
    const [signature, setSignature] = useState(null);
    const [status, setStatus] = useState(verificationStatus); // Default status
    const [reason, setReason] = useState(verificationRemark);
    ...
    const handleSubmit = async () => {
        if ((status === "rejected" || status === "reverification") && reason.trim() === "") {
            setError("Reason is required for rejection or reverification status.");
            return;
        }

        setError("");
        // Determine API URL based on status
        const apiUrl = status === "reverification"
            ? `${apiBaseUrl}/applicant/${application}/reverify/request`
            : `${apiBaseUrl}/applicant/${application}/${verifyType}`;
        ...
        try {
            const response = await apiRequestAsync("post", apiUrl, payload);
            if (response.status === 200) {
                fetchlist();
                toggle();
    ...
    return (
        <div className="container">
            ...
            {(status === "rejected" || status === "reverification") && (
